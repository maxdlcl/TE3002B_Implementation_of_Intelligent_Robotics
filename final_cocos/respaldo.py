...
class MainController(Node):
    def __init__(self):
        ...
        self.maneuver_duration = 1.9      # segundos
    ...
    def loop(self):
        ...
        # ─── 1. Cronómetro de maniobra (GO_AHEAD / TURN_*) ───
        if self.state in (StateFSM.GO_AHEAD, StateFSM.TURN_LEFT, StateFSM.TURN_RIGHT):
            if self.maneuver_t0 is not None and (now - self.maneuver_t0) >= self.maneuver_duration:
                ...
                # ←—— NUEVO: vuelve al modo 'ahead' por defecto
                if self.pending_maneuver is None:
                    self.pending_maneuver = 'ahead'

        # ─── 2. Señal STOP temporal ───
        ...
        # ─── 7. Flanco de subida /intersection ───
    ...
    # (ya NO se pone None aquí)


        # ─── 8. Publicar flags básicos ───
